...
const AdvancedSettingsModal: React.FC<AdvancedSettingsModalProps> = ({ books, onClose, onComplete }) => {
  ...
  const TabButton: React.FC<{ tab: ActiveTab; label: string }> = ({ tab, label }) => (
    <button onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${activeTab === tab ? 'bg-brand-secondary text-brand-text' : 'bg-slate-800 text-brand-subtle hover:bg-slate-700'}`}>
      {label}
    </button>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex justify-center items-center p-4">
      <div className="bg-brand-primary border border-slate-700 rounded-lg shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-grow p-4 bg-brand-secondary overflow-y-auto">
          {activeTab === 'books' && (
